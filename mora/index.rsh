  ...
  Bob.pay(wager)
    ...
    while ( outcome == TIE ) {
      ...
      Alice.only(() => {
        ...
        const _guessAlice = interact.getGuess(_thumbAlice);
     ...
     commit();
      ...
      Bob.only(() => {
        const _thumbBob = interact.getThumb();
        const _guessBob = interact.getGuess(_thumbBob);
        const thumbBob = declassify(_thumbBob); 
